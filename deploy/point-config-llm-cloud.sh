...
echo ""
curl -s -X POST "http://${IP}/api/llm/admin/probe" 2>/dev/null | head -c 120 || echo "(probe 需 admin 登录后在 admin-llm.html 点探活)"
echo ""
curl -s "http://${IP}/api/dashboard/summary-line" | python3 -c "import sys,json; d=json.load(sys.stdin); print('source:', d.get('source'))" 2>/dev/null || true
echo ""
echo "✅ 云上 DeepSeek 配置完成 · admin 验收 http://${IP}/admin-llm.html"
